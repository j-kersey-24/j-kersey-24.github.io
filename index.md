...
<br><br>
My group initally approached the problem with the intent to use supervised and unsupervised models to identify characteristics of customers which had high growth across the 2023-2024 data provided. My contribution was clustering, with poor results. Everyone in our group found the large and diverse dataset challenging to extract value from. One challenge was categorical variables with a high number of factors, such as the "sub trade channel" with 48 categories and the "primary group number" indicating customers as part of a chain which had 1021 unique values!
<br><br>
After other groupmates found that certain "frequent order type" factors were associated with high growth, we pivoted to another way SCC could utilize this information, and investigated potential treatment effects. I used matching with multiple linear regression to estimate average treatment effect (ATE) between using their old MyCoke Legacy online ordering platform vs their new MyCoke 360 platform. I also tried causal forest models to estimate individual treatment effects (ITEs) of Sales Representative vs the remaining frequent order types as control. The causal forest models again had poor results, including extremely low and even negative r-squared values! I suspect this was due to the categorical variables being turned into dummies and the data being scaled, resulting in a ton of categorical dummies and fewer columns which may have had more value.
<br><br>
A groupmate used t-learner treatment effect models with slightly better performance metrics. I analyzed the ITEs from the t-learner and calculated CATEs for some groups of customers. For example, for the set of all customers below 400 average gallons per year and not already frequently ordering via sales rep, they might increase an average of 10 gallons per year by switching to Sales Rep. However, customers below 400 average gallons per year, not already frequently ordering via sales rep, and in the sub-trade channel of Middle School could increase an average of 92 gallons per year by making the swap!
<br><br>
